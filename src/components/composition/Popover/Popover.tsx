...
export const Popover = forwardRef<HTMLDivElement, PopoverProps>(
  (
    ...
    const componentStyle: React.CSSProperties & Record<string, string> = {
      ...
      ...(maxWidth && {
        '--popover-custom-max-width': typeof maxWidth === 'number' ? `${maxWidth}px` : maxWidth,
      }),
      ...(minWidth && {
        '--popover-custom-min-width': typeof minWidth === 'number' ? `${minWidth}px` : minWidth,
      }),
      ...(maxHeight && {
        '--popover-custom-max-height': typeof maxHeight === 'number' ? `${maxHeight}px` : maxHeight,
      }),
      ...(minHeight && {
    ...
    const renderContent = () => {
      if (content && typeof content === 'object' && 'body' in content) {
        ...
        return (
          <>
            {header && <div className="popover-component__header">{header}</div>}
            <div className="popover-component__body">{body}</div>
            {footer && <div className="popover-component__footer">{footer}</div>}
            {actions && (
                  ...
                  ? actions.map((action, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {action.icon && (
                            <span className="popover-component__action-icon">{action.icon}</span>
                          )}
