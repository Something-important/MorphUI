  ...
  (
    ...
    const componentStyle: React.CSSProperties & Record<string, string> = {
      ...
      width: collapsed
        ...
        : typeof width === 'number'
          ? `${width}px`
          : resolveThemeValue(width) || width,
      minWidth: collapsed
        ? '60px'
        : typeof minWidth === 'number'
          ? `${minWidth}px`
          : resolveThemeValue(minWidth) || minWidth,
      maxWidth: collapsed
        ? '60px'
    ...
    const renderItems = React.useCallback(
      (items: SidebarItem[], level: number = 0): React.ReactNode => {
...
export const SidebarItemComponent = forwardRef<HTMLDivElement, SidebarItemProps>(
  (
    ...
    const hasChildren =
      propHasChildren !== undefined ? propHasChildren : item.children && item.children.length > 0;
    ...
    const content = (
      <>
        {/* Item Icon */}
        {item.icon && <span className="sidebar-component__item-icon">{item.icon}</span>}

        {/* Item Label */}
        <span className="sidebar-component__item-label">{item.label}</span>

        {/* Item Badge */}
        {item.badge && <span className="sidebar-component__item-badge">{item.badge}</span>}
